...
const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // 1. Récupérer d'abord l'ID via la route 'me' (robuste)
        const meRes = await api.get('/Profil/me');
        const idEtudiant = meRes.data.idEtudiant;

        if (idEtudiant) {
          // 2. Utiliser la route spécifique par ID demandée
          const res = await api.get(`/Profil/etudiant/${idEtudiant}`);
          setData(res.data);
        }
      } catch (error) {
  ...
  const update = async () => {
    try {
      ...
      // On s'assure que les données sont envoyées au format attendu par le DTO backend
      // Note: Le backend attend Nom, Prenom, Email, CodeApogee, Niveau, Filiere
      const res = await api.put('/Profil/me', updatedProfile);
      
      if (res.status === 200 || res.status === 204) {
        alert("Profil mis à jour avec succès !");
        // Optionnel : recharger les données pour refléter les changements
        const freshData = await api.get('/Profil/me');
      ...
      <div className="relative z-10 p-8 flex flex-col lg:flex-row gap-8 mt-4 max-w-5xl">
        ...
        <div className="flex-[2] bg-white rounded-3xl shadow-custom p-10">
          ...
          <form id="profileForm" className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            {/* Champs cachés nécessaires pour le DTO backend */}
            <input type="hidden" name="Email" value={data?.email} />
            <input type="hidden" name="CodeApogee" value={data?.codeApogee} />

            <FormField name="Nom" label="NOM" placeholder="Nom" defaultValue={data?.nomComplet?.split(' ')[0]|| ''} />
            <FormField name="Prenom" label="PRÉNOM" placeholder="Prénom" defaultValue={data?.nomComplet?.split(' ')[1]|| ''} />
            <FormField name="telephone" label="N° DE TÉLÉPHONE" placeholder="00000000" defaultValue={data?.telephone} />

            <div className="space-y-2">
              <label className="block text-xs font-bold text-gray-400 tracking-wider">LANGUE LOCALE</label>
              <div className="relative">
                <select name="langue_locale" value={data?.langue_locale} className="w-full bg-gray-50 border-none rounded-xl px-4 py-4 text-gray-800 font-medium appearance-none focus:ring-2 focus:ring-indigo-100 transition-all cursor-pointer">
            ...
            <FormField name="Filiere" label="FILIÈRE" placeholder="Ex : Informatique" defaultValue={data?.filiere} />
            <FormField name="Niveau" label="NIVEAU" placeholder="Ex : Master 2" defaultValue={data?.niveau} />
...
const FormField: React.FC<FormFieldProps> = ({ name, label, placeholder, defaultValue, type = "text" }) => (
  <div className="space-y-2">
