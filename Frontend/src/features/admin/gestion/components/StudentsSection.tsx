...
export function StudentsSection({ students, setStudents }: Props) {
  ...
  // CORRECTION : On précise que statut est de type 'Actif' | 'Inactif'
  const initialForm = { 
    codeApogee: '', 
    nom: '', 
    email: '', 
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        setIsLoading(true);
        // Assure-toi que l'URL "/Etudiant" correspond bien à ton backend
  ...
  const openEdit = (s: Student) => {
    setEditStudent(s);
    setForm({ 
      codeApogee: s.codeApogee,
      nom: s.nomComplet || s.nom || '', 
      email: s.email,
  ...
  const handleSave = async () => {
    if (!form.codeApogee || !form.nom || !form.email) {
      alert("Le code, le nom et l'email sont requis.");
      return;
    }
    ...
    try {
      if (editStudent) {
        const response = await api.put<Student>(`/Etudiant/${editStudent.id}`, payload);
        
        // Sécurité pour l'affichage : on s'assure d'avoir nomComplet
        const updatedStudent = {
            ...response.data,
            nomComplet: response.data.nomComplet || payload.nom
        };

        setStudents(prev => prev.map(s => s.id === editStudent.id ? updatedStudent : s));
      } else {
        const response = await api.post<Student>('/Etudiant', payload);
        
        // Sécurité pour l'affichage lors de l'ajout
        const newStudent = {
            ...response.data,
            nomComplet: response.data.nomComplet || payload.nom
        };
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-[#f1f5f9]">
              {isLoading ? (
                ...
                <tr><td colSpan={6} className="p-8 text-center text-slate-400">Aucun étudiant trouvé.</td></tr>
              ) : (
                paged.map(s => (
                  <tr key={s.id} className="hover:bg-[#fcfcfd]/80 transition-colors">
                    <td className="p-[16px_24px]"><span className="text-[13px] font-bold text-[#2563eb] hover:underline cursor-pointer">{s.codeApogee}</span></td>
                    ...
                    <td className="p-[16px_24px] text-[13px] text-[#1e293b] font-medium">{s.classeNom}</td>
                    <td className="p-[16px_24px]">
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editStudent ? 'Modifier l\'étudiant' : 'Ajouter un étudiant'}>
        <div className="flex flex-col gap-4">
          <div>
            ...
            <input className="w-full p-2.5 border border-[#e2e8f0] rounded-lg text-sm bg-white focus:border-[#2563eb] outline-none transition-all" value={form.codeApogee} onChange={e => setForm({ ...form, codeApogee: e.target.value })} />
          </div>
          <div>
          ...
          <div>
            ...
            <select className="w-full p-2.5 border border-[#e2e8f0] rounded-lg text-sm bg-white focus:border-[#2563eb] outline-none transition-all" value={form.classeNom} onChange={e => setForm({ ...form, classeNom: e.target.value })}>
          ...
          <div>
            <label className="block text-[11px] font-bold text-[#64748b] uppercase tracking-wider mb-1.5">Statut</label>
            <select className="w-full p-2.5 border border-[#e2e8f0] rounded-lg text-sm bg-white focus:border-[#2563eb] outline-none transition-all" value={form.statut} onChange={e => setForm({ ...form, statut: e.target.value as 'Actif' | 'Inactif' })}>
